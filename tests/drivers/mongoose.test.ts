...
describe("Mongoose Adapter", () => {
  ...
  describe("MongooseQueue", () => {
    ...
    it("should push and retrieve jobs", async () => {
      ...
      const jobId = await queue.addJob("test-job", {
        payload: { message: "Hello Mongoose!" },
      });
      expect(jobId).toBeDefined();
    ...
    it("should handle job failures and retries through the queue system", async () => {
      ...
      queue.setHandlers({
        "failing-job": async (job) => {
          ...
          if (job.payload.attemptNumber <= 2) {
            throw new Error(
              `Intentional failure on attempt ${job.payload.attemptNumber}`
            );
          }
      ...
      // Add jobs
      const failingJob1 = await queue.addJob("failing-job", {
        payload: { attemptNumber: 1 },
      });
      const failingJob2 = await queue.addJob("failing-job", {
        payload: { attemptNumber: 2 },
      });
      const failingJob3 = await queue.addJob("failing-job", {
        payload: { attemptNumber: 3 },
      });
      const successJob = await queue.addJob("success-job", {
      ...
      expect(job1Doc?.errorMessage).toContain(
        "Intentional failure on attempt 1"
      );
      expect(job2Doc?.errorMessage).toContain(
        "Intentional failure on attempt 2"
      );
    });
      ...
      queue.setHandlers({
        "long-job": async (job) => {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, job.payload.duration)
          );
  ...
  describe("Integration with Mongoose features", () => {
    it("provides the model to use however we want", async () => {
      const queue = new MongooseQueue({ name: "test-queue" });
      const MyQueueModel = queue.model;
      const jobId = await queue.addJob("test-job", {
        payload: { data: "test1" },
      });
      expect(await MyQueueModel.findById(jobId)).toBeDefined();
    });
