...
import { Queue, QueueError } from "../../src/core/queue.ts";
import type {
  JobMeta,
  QueueMessage,
  JobRequestFull,
...
class TestQueue extends Queue<TestJobs, JobRequestFull<any>> {
  public messages: Array<QueueMessage> = [];
  ...
  protected async pushMessage(
    payload: unknown,
    meta: JobMeta
  ): Promise<string> {
    const id = this.nextId.toString();
    this.nextId++;
    this.messages.push({ id, payload, meta });
    return id;
  }

  protected async reserve(_timeout: number): Promise<QueueMessage | null> {
  ...
  async status(_id: string): Promise<"waiting" | "reserved" | "done"> {
    return "done";
  }
...
describe("Queue", () => {
  ...
  describe("job processing", () => {
    ...
    it("should error when no handler is registered", async () => {
      ...
      const message: QueueMessage = {
        id: "1",
        payload: { data: "test data" },
        meta: { ttr: 300, name: "no-handler-for-this-job" },
      };
  ...
  describe("polling behavior", () => {
    ...
    class PollTrackingPlugin {
      ...
      getTimeBetweenPolls(index1: number, index2: number): number {
        if (
          index1 >= this.pollTimes.length ||
          index2 >= this.pollTimes.length
        ) {
          return -1;
        }
  ...
  describe("job completion methods", () => {
    ...
    it("should call failJob when no handler is registered", async () => {
      ...
      await queue.addJob("unregistered-job" as any, {
        payload: { data: "test data" },
      });
      ...
      expect((queue.failedJobs[0]?.error as any).message).toContain(
    ...
    it("should pass correct message data to completeJob", async () => {
      ...
      expect(completedJob.message.meta.name).toBe("test-job");
      expect(completedJob.message.payload).toEqual({ data: "test data" });
    });
    ...
    it("should pass correct message data to failJob", async () => {
      ...
      expect(failedJob.message.meta.name).toBe("test-job");
      expect(failedJob.message.payload).toEqual({ data: "failing job" });
    });
    ...
    it("should handle mixed success and failure jobs correctly", async () => {
      ...
      const failHandler = vi
        .fn()
        .mockRejectedValue(new Error("Intentional failure"));
      ...
      await queue.addJob("success-job", {
        payload: { data: "will succeed 2" },
      });
